#Copied from https://github.com/conceptofmind/vit-flax
...
class Trittention(nn.Module):
    ...
    def __call__(self, x):
        ...
        abcde = nn.Dense(features=5 * inner_dim)(x)
        a, b, c, d, e = jnp.split(abcde, 5, axis=-1)

        a, b, c, d, e = map(lambda t: rearrange(t, 'b n (h d) -> b n h d', h=self.heads), (a, b, c, d, e))

        attn_pattern = einsum("bsnh, btnh, bqnh -> bnstq", c, b, a)
        attn_score = rearrange(attn_pattern, "b n p1 p2 p3 -> b n p1 (p2 p3)") / scale
        attn_score = nn.softmax(attn_score, axis=-1)
        attn_score = rearrange(attn_score, "b n p1 (p2 p3) -> b n p1 p2 p3", p2=x.shape[1], p3=x.shape[1])

        z = einsum('bnqlr, blnd -> bnqd', attn_score, e) + einsum('bnqlr, brnd -> bnqd', attn_score, d)
        z = rearrange(z, 'b n q d -> b q (n d)')

        out = nn.Dense(features = self.dim)(z)
        to_out = nn.Dropout(rate = self.dropout)(out, deterministic = False)
...
class Triformer(nn.Module):
    ...
    def __call__(self, x):
        ...
        for _ in range(self.depth):
            layers.append([
                Residual(PreNorm(Trittention(self.dim, self.heads, self.dim_head, self.dropout))),
                Residual(PreNorm(FeedForward(self.dim, self.mlp_dim, self.dropout)))
            ])
...
class ViTtri(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = Triformer(self.dim, self.depth, self.heads, self.dim_head, self.mlp_dim, self.dropout)(x)
